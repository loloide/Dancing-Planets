...
let canvasSize = 500; // Adjust based on available space

function setup() {
    createP("Dancing Planets").style("font-size", "24px");

    createCanvas(canvasSize, canvasSize);

    let div1 = createDiv().class("section");
    createP(
        "Select the planets and a skip value (in case you need less lines)."
    ).parent(div1);
    ...
    let showPlanetsButton = createButton("Toggle show planets")
        .class("spaced-button")
        .parent(div1);
    showPlanetsButton.mousePressed(() => {
    ...
    let div4 = createDiv().id("credit");

    let creditText = createSpan("Made with love by ").parent(div4);
    createA("https://github.com/loloide", "Loloide", "_blank").parent(div4);
}
...
function savePlanets() {
    saveCanvas(
        planets[selectA.selected()].name +
            " - " +
            planets[selectB.selected()].name,
        "png"
    );
}
